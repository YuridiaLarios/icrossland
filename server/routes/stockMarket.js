...
router.get("/history/:id", async function(req, res) {
  await axios.get(`https://www.worldtradingdata.com/api/v1/history?symbol=${req.params.id}&date_from=2019-05-01&sort=newest&sort=oldest&api_token=${process.env.WTD_API}`).then((response) => res.json(response.data.history)).catch(error => res.json(error));
});
